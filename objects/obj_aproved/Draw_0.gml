/// @description
image_alpha = 1;

if (selected == true) {
	image_alpha = 0.8;
	var _aproved_string = "";
	var effects = string_split(obj_NPC.card_data.aceito, ";");
	/*
    for (var i = 0; i < array_length(effects); i++) {
        var effect = string_trim(effects[i]);
        if (effect == "") continue;
        
        var space_pos = string_pos(" ", effect);
        if (space_pos == 0) continue; // Pular
        
        // Quantidade a ser extraida
        var amount_str = string_copy(effect, 1, space_pos-1);
        var amount = real(amount_str);
        
        // Recurso em questão
        var resource = string_trim(string_copy(effect, space_pos+1, string_length(effect)));
        resource = string_lower(resource); // converte pra lowercase só pra garantir 
        
        // aplicar
        switch (resource) {
            case "din":
                _aproved_string += "dinheiro ";
				if (amount >=0){_aproved_string += "+ "} else{_aproved_string += "- "}
				_aproved_string += string(abs(amount));
                break;
            case "aprov":
                _aproved_string += "aprovacao ";
				if (amount >=0){_aproved_string += "+ "} else{_aproved_string += "- "}
				_aproved_string += string(abs(amount));
                break;
            case "inf":
               _aproved_string += "infraestrutura ";
			   if (amount >=0){_aproved_string += "+ "} else{_aproved_string += "- "}
				_aproved_string += string(abs(amount));
                break;
            case "com":
                _aproved_string += "comida ";
				if (amount >=0){_aproved_string += "+ "} else{_aproved_string += "- "}
				_aproved_string += string(abs(amount));
                break;
            default:
                show_debug_message("Que porra de recurso é esse aqui: " + resource);
                break;
        }
		if (i + 1 < array_length(effects)){_aproved_string += "| " }
    }
	
	draw_text(x + 100, y + 50, _aproved_string)*/
	
	var _w = string_width(obj_NPC.card_data.aprovar) + 40;
	
	draw_sprite_ext(spr_bar, 0, RESOLUTION_WIDTH/2 - string_width(obj_NPC.card_data.aprovar)/2 - 20, 400 - 10, _w/sprite_get_width(spr_bar), 1, 0, c_white, 1);
	draw_text_color(RESOLUTION_WIDTH/2 - string_width(obj_NPC.card_data.aprovar)/2 + 1, 400 + 1, obj_NPC.card_data.aprovar, c_black, c_black, c_black, c_black, 1);
	draw_set_color(c_white);
	draw_text_transformed(RESOLUTION_WIDTH/2 - string_width(obj_NPC.card_data.aprovar)/2, 400, obj_NPC.card_data.aprovar, 1, 1, 0);
};

draw_self();